...
def extract_milestone(input_text: str) -> Tuple[str, str, str]:
    pattern = (
        r"\s*Thought: (.*?)\n+Milestone: ([^\n\(\) ]+).*?\n+"
    )
    ...
    if not match:
        raise ValueError(f"Could not extract milestone from input text: {input_text}")

    thought = match.group(1).strip()
    milestone = match.group(2).strip()
    action = match.group(3).strip()
    return thought, milestone, action  
...
class customReActOutputParser(BaseOutputParser):
    """Custom ReAct Output parser with milestone"""
    ...
    def parse(self, output: str, is_streaming: bool = False) -> BaseReasoningStep:
        ...
        1. If the agent can reach a milestone conclusion without calling a tool:
            ```
            Thought: <thought>
            Milestone: <Milestone conclusion>
            Action: <action>
            ```            

        2. If the agent need to use a tool to answer the question:
            ```
            Thought: <thought>
            Action: <action>
            Action Input: <action_input>
            ```
        3. If the agent can answer the question without any tools:
            ```
            Thought: <thought>
            Answer: <answer>
            ```
        ...
        # A "Milestone" should take priority over an "Action"
        if "Milestone:" in output:
            thought, milestone = extract_milestone(output)
            return MilestoneReasoningStep(
                thought=thought,
                milestone=milestone,
            )
